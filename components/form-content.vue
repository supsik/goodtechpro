    ...
    <div class="tab-content">
      <div v-if="activeTab === 'questionnaire'" class="questionnaire-tab">
        <div v-if="!isQuestionnaireStarted" class="questionnaire-start">
          <p class="intro-text">
            Пройдите опрос, чтобы получить предварительную оценку проекта и
            перейти к форме обратной связи.
          </p>
          <button
            type="button"
            class="submit-btn start-btn"
        ...
        <span class="form-content__info"
          >* Опрос и его результат не являются публичной офертой</span
        >
      </div>
        ...
        <div v-if="!isFormSubmitted" class="feedback-form-container">
          ...
          <form @submit.prevent="submitFeedback">
            ...
            <div class="contact-requirement-note">
              <span>ℹ️</span>
              <p>
                Укажите email или телефон для связи (обязательно хотя бы одно)
              </p>
            </div>
...
const submitFeedback = async () => {
  ...
  try {
    ...
    const errorMessage = error.data?.statusMessage || "Произошла ошибка при отправке. Пожалуйста, попробуйте еще раз."
    alert(errorMessage)
  } finally {
